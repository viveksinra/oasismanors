...
function Employee() {
  ...
  useEffect(() => {
    async function fetchAllData() {
      setLoading(true);
      let response = await employeeService.getEmployee(
        "api/v1/employee/basic/getEmployee/getAll",
        ""
      );
  ...
  const columns = [
    {
      ...
      width: 60,
      renderCell: (props) => (
        <Avatar alt={props?.row?.firstName} src={props?.row?.userImage} />
      ),
    },
    {
      field: "firstName",
    ...
    },
    {
      ...
      renderCell: (props) => (
        <Button
          startIcon={<FiEdit />}
          onClick={() => router.push(`/dashboard/employee/${props?.row?._id}`)}
        >
          Edit
        </Button>
      ),
    ...
    <main>
      <Box sx={{ background: "#fff", borderRadius: "10px", width: "100%" }}>
        ...
        {loading ? (
          ...
          <NoResult label="No Employee Available." />
        ) : (
          <DataGrid
            rows={rows}
            columns={columns}
      ...
      <AppBar
        position="fixed"
        sx={{ top: "auto", bottom: 0, background: "#d6f9f7" }}
      >
